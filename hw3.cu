...
float reduceMinmax(const float* const d_in, const size_t numRows, const size_t numCols, int minmax) {
    ...
    while(true) {
        checkCudaErrors(cudaMalloc(&d_curr_out, sizeof(float) * getMaxSize(currSize, BLOCK_SIZE)));
        ...
        currSize = getMaxSize(currSize, BLOCK_SIZE);
    }
                                  ...
                                  const size_t numBins)
{
  ...
  /*Here are the steps you need to implement
    ...
    dim3 hist_block_dim(getMaxSize(size, thread_dim.x));
    ...
    dim3 scan_block_dim(getMaxSize(numBins, thread_dim.x));
